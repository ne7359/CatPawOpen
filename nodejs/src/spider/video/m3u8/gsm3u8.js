...
let url = 'https://api.guangsuapi.com/api.php/provide/vod/josn';
let categories = ['动作片', '喜剧片', '爱情片', '科幻片', '剧情片', '恐怖片', '战争片', '动漫电影', '大陆剧', '欧美剧', '港澳剧', '韩剧', '日剧', '台湾剧', '泰剧', '伦理片', '综艺', '动漫', '记录片', '体育赛事', '短剧'];
...
async function init(inReq, _outResp) {
    // url = inReq.server.config.gsm3u8.url;  // 这里可以移除与配置文件相关的部分，直接使用内部定义的 url   注：gsm3u8为key值
    // categories = inReq.server.config.gsm3u8.categories;  // 这里可以移除与配置文件相关的部分，直接使用内部定义的 categories    注：gsm3u8为key值
    return {};
}
