...
let url = 'https://cjhwba.com/api.php/provide/vod/';
let categories = ['冒险片', '剧情片', '动作片', '动画电影', '喜剧片', '奇幻片', '恐怖片', '悬疑片', '惊悚片', '歌舞片', '灾难片', '爱情片', '科幻片', '犯罪片', '经典片', '网络电影', '战争片', '伦理片', '同性片', '电视剧', '欧美剧', '日剧', '韩剧', '台剧', '泰剧', '国产剧', '港剧', '新马剧', '其他剧', '纪录片', '欧美动漫', '日韩动漫', '国产动漫', '新马泰动漫', '港台动漫', '其他动漫', '国产综艺', '日韩综艺', '欧美综艺', '新马泰综艺', '港台综艺', '其他综艺', '短剧', '预告片'];
...
async function init(inReq, _outResp) {
    // url = inReq.server.config.jinyingyun.url;  // 这里可以移除与配置文件相关的部分，直接使用内部定义的 url    注：jinyingyun为key值
    // categories = inReq.server.config.jinyingyun.categories;  // 这里可以移除与配置文件相关的部分，直接使用内部定义的 categories    注：jinyingyun为key值
    return {};
}
...
async function home(_inReq, _outResp) {
    ...
    for (const cls of data.class) {
        ...
        for (const category in categories) {
            if (categories[category].includes(n)) {
                classes.push({
    ...
    classes = classes.sort((a, b) => {
        ...
        return categoryOrder.indexOf(a.type_name) - categoryOrder.indexOf(b.type_name);
    });
